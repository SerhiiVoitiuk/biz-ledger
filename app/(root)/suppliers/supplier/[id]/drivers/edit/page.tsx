...
import { getSupplierDriver } from "@/lib/data/supplier";
import SupplierDriverEditForm from "@/components/supplier/SupplierDriverEditForm";

const Page = async ({ params }: { params: Promise<{ id: string }> }) => {
  const id = (await params).id;
  const session = await auth();
  const drivers = await getSupplierDriver(id);
