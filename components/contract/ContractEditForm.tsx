...
import { updateContract } from "@/lib/actions/сontract";
...
const ContractEditForm = ({
  ...
  const form = useForm<z.infer<typeof contractsSchema>>({
    ...
    defaultValues: {
      ...
      executionPeriod: contract?.executionPeriod || "",
    },
  });

  const onSubmit = async (values: z.infer<typeof contractsSchema>) => {
    setIsLoading(true);
    const result = await updateContract(values, contract.id as string, userId as string);
    setIsLoading(false);
            ...
            <FormItem className="flex flex-col gap-1">
              ...
              <FormControl>
                <Input
                  ...
                  {...field}
                  className="w-full min-h-12 mt-1 font-bold placeholder:font-normal 
                  text-[#11191f]
