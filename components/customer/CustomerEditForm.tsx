...
import { customersSchema } from "@/lib/validations";
import { updatedCustomer } from "@/lib/actions/customer";

const CustomerEditForm = ({ customer }: { customer: CustomerById }) => {
  const [isLoading, setIsLoading] = useState(false);
  const router = useRouter();
  const userId = customer.userId as string;
  ...
  const onSubmit = async (values: z.infer<typeof customersSchema>) => {
    setIsLoading(true);
    const result = await updatedCustomer(customer.id, values, userId);
    setIsLoading(false);
          ...
          render={({ field }) => (
            <FormItem className="flex flex-col gap-1">
              ...
              <FormControl>
                <Input
                  required
                  placeholder="Повна назва замовника"
                  {...field}
          ...
          render={({ field }) => (
            <FormItem className="flex flex-col gap-1">
              ...
              <FormControl>
                <Input
                  required
                  placeholder="Код ЄДРПОУ замовника"
                  {...field}
                  className="w-full min-h-12 mt-1 text-xl font-bold 
                  placeholder:font-normal text-dark
                  placeholder:text-dark focus-visible:ring-0 focus-visible:shadow-none
          ...
          render={({ field }) => (
            <FormItem className="flex flex-col gap-1">
              ...
              <FormControl>
                <Input
                  required
                  placeholder="Номер телефону замовника"
                  {...field}
                  className="w-full min-h-12 mt-1 text-xl font-bold 
                  placeholder:font-normal text-dark
                  placeholder:text-dark focus-visible:ring-0 focus-visible:shadow-none
          ...
          render={({ field }) => (
            <FormItem className="flex flex-col gap-1">
              ...
              <FormControl>
                <Input
                  required
                  placeholder="Електронна адреса замовника"
                  {...field}
                  className="w-full min-h-12 mt-1 text-xl font-bold 
                  placeholder:font-normal text-dark
                  placeholder:text-dark focus-visible:ring-0 focus-visible:shadow-none
