...
import { deleteSupplierDrivers } from "@/lib/actions/supplier";
...
const DriversInfo = ({
  ...
  const handleDelete = async (supplierId: string) => {
    try {
      setIsLoading(true);
      const result = await deleteSupplierDrivers(supplierId);
      if (result.success) {
        toast.success("Водія (-їв) успішно видалено");
        router.refresh();
        setOpen(false);
      } else {
      ...
      toast.error("Помилка при видаленні Водія (-їв)");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex flex-row gap-3">
        ...
        >
          <Link
            href={`/suppliers/supplier/${supplierInfoId}/drivers/edit`}
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Видалити водія (-їв)</DialogTitle>
              <DialogDescription>
                Ви впевнені, що хочете видалити водія (-їв)?
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              <Button
